...
(* TODO: think about how to do bools later *)
let t_bool = TCon (Tycon ("Bool", Star))
...
type literal =
  ...
  | LitRat of float
  | LitStr of string
  (* TODO: design booleans as union types *)
  | LitBool of bool
...
type expr =
  ...
  (* let expressions in haskell allows you to bind multiple variables using a single "let" keyword.
     That's why we have a list of expl's and impl's in a single let expression. *)

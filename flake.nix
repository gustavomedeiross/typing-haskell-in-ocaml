    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        ...
        packages = {
          ...
          #     $ nix run -- <args?>
          #
          default = self.packages.${system}.thio;

          thio = ocamlPackages.buildDunePackage {
            pname = "thio";
            version = "0.1.0";
            duneVersion = "3";
            src = sources.ocaml;
        ...
        #
        checks = {
          # Run tests for the `thio` package
          thio =
            let
            ...
            self.packages.${system}.thio.overrideAttrs
              (oldAttrs: {
                name = "check-${oldAttrs.name}";
                doCheck = true;
                buildPhase = patchDuneCommand oldAttrs.buildPhase;
                checkPhase = patchDuneCommand oldAttrs.checkPhase;
                # skip installation (this will be tested in the `thio-app` check)
